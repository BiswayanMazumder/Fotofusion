...
class Searcheduserfollowers extends StatefulWidget {
  final String userid;
  Searcheduserfollowers({required this.userid});

  @override
  State<Searcheduserfollowers> createState() => _SearcheduserfollowersState();
}

class _SearcheduserfollowersState extends State<Searcheduserfollowers> {
  ...
  Future<String> fetchUsernameFromId(String userId) async {
    try {
      final docSnapshot = await _firestore.collection('User Details').doc(widget.userid).get();
  ...
  Future<String> fetchProfilePictureUrl(String userId) async {
    try {
      final docSnapshot = await _firestore.collection('profile_pictures').doc(widget.userid).get();
      if (docSnapshot.exists) {
  ...
  Future<void> fetchFollowersAndUsernames() async {
    ...
    try {
      ...
      final userSnapshot = await _firestore.collection('User Details').doc(widget.userid).get();
      ...
      final userSnapshotAfter = await _firestore.collection('User Details').doc(widget.userid).get();
